...
contract WrappedVaultDecimalOffsetTest is Test {
    ...
    ERC20 token = token1; // LEAVE THE UNDERLYING ASSET WITH 18 DECIMALS
    ...
    function setUp() public {
        rewardToken = rewardToken2; // LEAVE THE REWARD TOKEN AS IS, WITH SIX DECIMALS
        testVault = testVault2; // USE THE DECIMAL OFFSET VAULT
        // IF IN THE DecimalOffsetERC4626 YOU CHANGE THE OFFSET TO ZERO, BUT INCREASE THE VAULT DECIMALS BY 6, THE PROBLEM DISAPPEARS
    ...
    function testClaimLocal_Debug() public {
        uint256 depositAmount = 1_000 * 10 ** token.decimals();
        uint32 timeElapsed = 15 days;

        uint256 rewardAmount = 500 * 10 ** rewardToken.decimals();
